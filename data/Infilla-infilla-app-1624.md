This PR fully removes OpenSearch from the project. This includes deleting all related code, configuration, and documentation, as well as removing OpenSearch from local development environments. Functionality that previously relied on OpenSearch is now handled by other services or removed entirely.

### OpenSearch Removal and Indexer Changes

The primary goal of this PR is to completely remove OpenSearch from the project. This involves deleting all related OpenSearch configurations, client services, and indexer logic. The `thread` indexer, which was previously responsible for indexing thread data into OpenSearch, has been modified to be a no-op, meaning it will now log that it skipped indexing threads instead of performing any actual indexing. This ensures that any remaining calls to the indexer do not cause errors while the system transitions away from OpenSearch.
...
+      logger.error('Error in thread indexing no-op', e);
     ...
     .finally(() => {
@@ -51,42 +50,6 @@ export const indexManyThreads = async ({
...
-  const searchClient = await getSearchClient();
   ...
   const threads = await prisma.thread.findMany({
     where,
@@ -65,42 +28,6 @@ export const indexManyThreads = async ({
   });
...
### Search Query Logic Refactor

The `thread-list.server.ts` file previously contained a function `_toOpenSearchQuery` responsible for converting `ThreadListFilterState` into an OpenSearch query. This function has been removed, as OpenSearch is no longer used for thread searching. The application will now rely solely on `toPrismaQuery` for database interactions, simplifying the data access layer.
...
@@ -296,156 +295,6 @@ export const searchThreads = async <ThreadIncludeT extends Prisma.ThreadInclude>
   }
 };
...
This section removes all OpenSearch-related infrastructure definitions and deployment steps. The `SearchDomain` construct, which previously managed the OpenSearch cluster within the AWS CDK, has been deleted. Correspondingly, `InfillaApplication` no longer imports or uses `SearchDomain`, simplifying the application's infrastructure dependency graph. `cdk.json` also removes a related OpenSearch setting.

````diff
diff --git a/cdk.json b/cdk.json
index 48d8f6128..1b81e8252 100644
--- a/cdk.json
+++ b/cdk.json
@@ -48,7 +48,6 @@
...
index e06e1d081..dbec1f31 100644
...
@@ -138,17 +136,14 @@ export class InfillaAppStack extends Stack {
...
-
     ...
     // ========================================================================
     if (!props.env?.region) {
       throw new Error('No region set in CDK environment');
     }
-
     this.application = new InfillaApplication(this, 'Application', {
       mode: props.mode,
-      searchDomain: this.searchDomain,
+
       sentry: props.sentry,
       domainName: props.domainNames.app,
       rootUrl: `https://${props.domainNames.app}`,
@@ -165,10 +160,8 @@ export class InfillaAppStack extends Stack {
       radar: props.radar,
     });
...
### Build Scripts and Local Development Environment Updates

All references to OpenSearch in build scripts, local development configurations, and `package.json` have been removed. This includes:

- Deleting OpenSearch migrations scripts and their associated commands (`npm run opensearch:migrate`, `npm run opensearch:reindex`).
- Removing the OpenSearch service from `docker-compose.base.yml`.
- Updating documentation (`AGENTS.md`, `README.md`, `WARP.md`, `docs/disaster.md`, `docs/environment.md`) to reflect the removal of OpenSearch.
...
@@ -51,10 +48,6 @@ npm run prettier:fix         # Fix formatting on changed files
...
@@ -69,7 +62,7 @@ npm run cdk:deploy-production # Deploy to production AWS environment
 # Build operations
...
@@ -79,7 +72,6 @@ npm run build:docker         # Build Docker image
...
@@ -87,6 +79,7 @@ npm run build:docker         # Build Docker image
 ### Key Directories
...
@@ -99,6 +92,7 @@ npm run build:docker         # Build Docker image
...
@@ -107,9 +101,11 @@ npm run build:docker         # Build Docker image
...
@@ -120,6 +116,7 @@ The application has two distinct form systems:
...
@@ -127,24 +124,29 @@ The application has two distinct form systems:
 ### Key Patterns
...
@@ -152,19 +154,23 @@ The application has two distinct form systems:
...
+2. Remix imports
 ...
 5. Relative imports
@@ -179,15 +185,19 @@ Organize imports in this order:
...
@@ -195,19 +205,23 @@ Organize imports in this order:
...
@@ -216,3 +230,132 @@ - The codebase prioritizes junior developer accessibility
...
### Deletion of OpenSearch Specific Files and Dependencies

All remaining OpenSearch-specific files, such as `opensearch/migration.ts`, `opensearch/schema.ts`, and `opensearch/types.ts`, have been deleted. The `@opensearch-project/opensearch` package and its related dependencies have also been removed from `package.json` and `package-lock.json`, completing the cleanup of the OpenSearch integration.
...
These are small, isolated changes that contribute to the overall removal of OpenSearch but do not fit into larger thematic groupings.
...
--- a/server-lambda.js
+++ b/server-lambda.js
@@ -3,17 +3,8 @@ import { createRequestHandler } from '@remix-run/architect';
