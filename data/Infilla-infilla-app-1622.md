...
- Removes all OpenSearch dependencies, infrastructure, and tooling, effectively disabling full-text search and background indexing throughout the stack. **This is a breaking change** for any features that previously relied on OpenSearch-backed queries.
- Simplifies runtime behavior by turning former indexing flows into no-ops and deleting the OpenSearch client layer, scripts, and Lambdas.
- Updates documentation, local development tooling, Docker images, and infrastructure to reflect the absence of OpenSearch, along with pruning related npm packages.

## Removing OpenSearch from developer documentation and setup guides

All developer-facing docs now describe a stack without OpenSearch so new contributors aren't prompted to configure or run the retired service.
...
## Application runtime no longer performs search indexing

Runtime flows now skip search indexing entirely and drop OpenSearch-specific helpers, reflecting the removal of the search backend.
...
## Deleting OpenSearch service and supporting scripts

The codebase removes the OpenSearch client, schemas, migration scripts, and associated command runner Lambda because these assets are no longer used.
...
## Infrastructure cleanup: removing OpenSearch from builds, CDK, and Docker

Infrastructure definitions no longer provision or reference OpenSearch, ensuring deployments and local environments stay consistent with the runtime changes.
...
With OpenSearch gone, the project drops the client package and its transitive dependencies from both `package.json` and the lockfile.
...
A handful of small cleanups remove OpenSearch-specific mentions from tooling comments.
