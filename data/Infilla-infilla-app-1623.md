# Removal of OpenSearch Search Infrastructure

This PR completely removes OpenSearch dependency from the codebase, including all search indexing functionality, deployment infrastructure, and related configuration. The search feature is being deprecated in favor of database-backed filtering and querying.

## Removal of OpenSearch Service Infrastructure

The OpenSearch cluster infrastructure and related AWS CDK constructs are being removed from the deployment layer. This includes the SearchDomain construct that managed the OpenSearch cluster in AWS.
...
The `InfillaApplication` construct is updated to remove the searchDomain parameter and all associated references:
...
The top-level stack that instantiates the application is also updated to remove SearchDomain creation and related configurations:
...
## Removal of OpenSearch Client and Search Configuration

The OpenSearch client service that handled connections to the search cluster is completely removed:
...
## Conversion of Search Indexing to No-ops

Thread indexing operations are converted to no-ops since the search infrastructure is being removed. The indexing functions now simply return without performing any actual indexing:
...
## Removal of OpenSearch Schema and Type Definitions

The OpenSearch index schema definitions and related type definitions are removed since they're no longer needed:
...
## Removal of OpenSearch Query Building and Search Script

The complex OpenSearch query builder that was used for fulltext search queries is removed from the thread list query module:
...
## Removal of OpenSearch Scripts and Deployment Lambdas

The OpenSearch migration command scripts and the Lambda function that ran them are removed:
...
## Removal of OpenSearch from Docker and Lambda Initialization

OpenSearch Docker service is removed from the local development environment, and the search client initialization is removed from Lambda startup:
...
The OpenSearch npm package and its dependencies are removed from the project dependencies:
...
@@ -32,8 +32,6 @@
     ...
     "cdk:deploy-production": "aws sts get-caller-identity --profile production && npm run pre-deploy '*prod*' && SENTRY_RELEASE=$(git rev-parse --short HEAD) MODE=production dotenv -e .env.production -- cdk --profile production deploy --all  && npm run cdk:cleanup",
...
Documentation files are updated to remove references to OpenSearch setup, configuration, and deployment procedures:

````diff
diff --git a/docs/environment.md b/docs/environment.md
index 77255f72..b673245df 100644
...
The README is updated to remove OpenSearch migration steps and references from development instructions:
...
The WARP.md file is updated to remove OpenSearch commands and references from development guidance:
...
The AGENTS.md file is updated to remove the OpenSearch migration command:
...
Minor updates to remove OpenSearch references from deployment procedures and disaster recovery documentation:
...
@@ -1,6 +1,5 @@
 ...
 import { type ISecret } from 'aws-cdk-lib/aws-secrets-manager';
...
Finally, the test mock for the thread indexer is removed since the indexing now happens without side effects:
...
Another comment update in the forum import script:
